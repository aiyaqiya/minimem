...
//开始显示
    ...
    var _this=this;
    wx.request({
      url: "http://www.jiyilive.com/internetResources/queryByType",//"http://123.56.69.131/suport/getUrl.php",//
      data: { "type": 2, "activityId": app.globalData.selfActiveId},
      method:"post",
      header:{
      ...
      success: function (res) {
        ...
        wx.getImageInfo({
          src: res.data.data[0].url,//res.data,//
          success: function(re){
            _this.setData({
              timg:re
            });
  ...
  makeStep:function(e){
   ...
   // if (!this.checkCanMove(pos)){return;}    
    if (this.checkPass()){        
      ...
      console.log(ggkk);
      wx.request({
        url: "http://www.jiyilive.com/activityUser/update",
        ...
        success: function (res) {          
          wx.showToast({//layer.msg类的提示框，自动隐藏
            ...
            duration: 1000,
            success: function () {
              setTimeout(() => { wx.redirectTo({ url: '../index/index?id='+app.globalData.selfActiveId }); }, 1000);
            }
          });
        }
      });  
